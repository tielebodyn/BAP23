...
<div class="js-file-input-wrapper">
  <div class="mb-2">
    ...
    <input data-max-amount="{{ $count }}" type="file" @if ($count > 1) multiple @endif
      ...
      {{ $attributes->merge(['class' => 'js-file-input text-sm text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-violet-50 file:text-violet-700 hover:file:bg-violet-100']) }}
      max="{{ $count }}" />
  ...
  <div
    class="preview-wrapper h-fit rounded border grid gap-1 p-2 h-80
  @if ($count > 1) grid-cols-2
 w-full @else
w-1/2 @endif
  ">
    @if ($default)
